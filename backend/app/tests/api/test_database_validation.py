...
class TestMissingRowDataValidation:
    ...
    def test_table_id(self, client, auth_headers_user1):
        """
        Create a test table for row insertion validation.
...
class TestEmptyRowDataValidation:
    """
    Test suite for validating empty row_data returns 422.

    Tests various empty states:
    - Empty array
    - Empty object (when object expected)
    """

    @pytest.fixture
    def test_table_id(self, client, auth_headers_user1):
        """
        Create a test table for empty row_data validation.
        ...
        unique_suffix = uuid.uuid4().hex[:8]
        table_request = {
            "table_name": f"test_empty_validation_{unique_suffix}",
            "description": "Table for empty row_data validation testing",
            "schema": {
                "fields": {
                    "name": {"type": "string", "required": True},
    ...
    def test_empty_row_data_array_returns_422(
        self, client, auth_headers_user1, test_table_id
    ):
        """
        Test POST with empty row_data array returns 422.

        Given: POST request with row_data as empty array []
        When: Attempting to insert rows
        Then: Should return HTTP 422 with appropriate error
        And: Error should indicate no rows provided
        """
        request_body = {
            "row_data": []
        }
        ...
        # Accept either 422 for validation error or 201 with 0 inserts
        # depending on business logic implementation
        if response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY:
            data = response.json()
            assert "error_code" in data
            assert "detail" in data
            # Error should be about empty/invalid data
            ...
            # If implementation allows empty array, verify response
            assert response.status_code == status.HTTP_201_CREATED
            data = response.json()
            assert data["inserted_count"] == 0
            assert len(data["rows"]) == 0

    def test_row_data_with_empty_object_handles_validation(
        self, client, auth_headers_user1, test_table_id
    ):
        """
        Test POST with row_data as empty object handles validation.

        Given: POST request with row_data as empty object {}
        When: Attempting to insert row with no fields
        Then: Should validate against table schema
        And: Return appropriate error if required fields missing
        """
        request_body = {
            "row_data": {}
        }
        ...
        # Should either succeed or fail based on schema validation
        # Since 'name' is required, this should fail
...
class TestNullRowDataValidation:
    ...
    def test_table_id(self, client, auth_headers_user1):
        """
        Create a test table for null row_data validation.
...
class TestInvalidRowDataTypeValidation:
    ...
    def test_table_id(self, client, auth_headers_user1):
        """
        Create a test table for type validation.
...
class TestErrorResponseFormat:
    ...
    def test_table_id(self, client, auth_headers_user1):
        """
        Create a test table for error format validation.
...
class TestBatchInsertValidation:
    """
    Test suite for validating batch insert operations.

    Tests validation for:
    - Array of valid objects (should succeed)
    - Array with invalid objects (should fail with details)
    - Mixed valid/invalid objects (should fail with details)
    """

    @pytest.fixture
    def test_table_id(self, client, auth_headers_user1):
        """
        Create a test table for batch insert validation.
        ...
        unique_suffix = uuid.uuid4().hex[:8]
        table_request = {
            "table_name": f"test_batch_validation_{unique_suffix}",
            "description": "Table for batch insert validation testing",
            "schema": {
                "fields": {
                    "name": {"type": "string", "required": True},
                    "value": {"type": "integer", "required": False}
    ...
    def test_batch_insert_with_valid_array_succeeds(
        self, client, auth_headers_user1, test_table_id
    ):
        """
        Test batch insert with valid array of objects succeeds.

        Given: POST request with row_data as array of valid objects
        When: Attempting to insert multiple rows
        Then: Should return HTTP 201 CREATED
        And: All rows should be inserted successfully
        """
        request_body = {
            "row_data": [
                {"name": "Alice", "value": 10},
    ...
    def test_batch_insert_with_invalid_types_in_array_returns_422(
        self, client, auth_headers_user1, test_table_id
    ):
        """
        Test batch insert with invalid types in array returns 422.

        Given: POST request with row_data array containing non-objects
        When: Attempting to insert rows
        Then: Should return HTTP 422 with validation error
        """
        request_body = {
            "row_data": [
                {"name": "Valid"},
        ...
        # Should reject entire batch if any item invalid
        if response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY:
            data = response.json()
            assert "error_code" in data
            assert "detail" in data
